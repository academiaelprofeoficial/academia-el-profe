...
// ============================================================
// VideoPlayer ÔÇö Reproductor de video profesional
...
export function VideoPlayer({ videoUrl, webmUrl, titulo, posterUrl, isFree = false, onProgress, onComplete }: VideoPlayerProps) {
  ...
  // ÔöÇÔöÇ iOS Detection (before any early returns) ÔöÇÔöÇ
  ...
  // Picture-in-Picture Autom├ítico al cambiar de pesta├▒a/minimizar
  useEffect(() => {
    ...
    const handleVisibilityChange = async () => {
      ...
      if (document.hidden && !isPipActive) {
        // Usuario cambi├│ de pesta├▒a o minimiz├│ - Activar PiP autom├íticamente
        pipTimeout = setTimeout(async () => {
          try {
            ...
            console.log('PiP autom├ítico no disponible:', error);
          }
        }, 100);
      } else if (!document.hidden && isPipActive) {
        // Usuario regres├│ a la pesta├▒a - Salir de PiP
  ...
  // ÔöÇÔöÇ Pinch-to-zoom ÔåÆ fullscreen landscape (like YouTube mobile) ÔöÇÔöÇ
  useEffect(() => {
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      // If pinching out (zoom in) beyond 30px threshold ÔåÆ go fullscreen
  ...
  // ÔöÇÔöÇ Archivo de video directo (Sanity upload: MP4, MOV, WebM) ÔöÇÔöÇ
  if (videoUrl) {
    ...
    const toggleFullscreen = useCallback(async () => {
      ...
      // iOS: use native video fullscreen (webkitEnterFullscreen) ÔÇö auto landscape
    ...
    // ÔöÇÔöÇ Seek helpers (shared by mouse + touch) ÔöÇÔöÇ
    ...
    // Control de tama├▒o con forzado de DOM
    ...
    // ÔöÇÔöÇ Canvas overlay anti-grabaci├│n ÔöÇÔöÇ
    useEffect(() => {
      const video = videoRef.current;
        ...
        if (!video.paused && !video.ended) {
          if (!isRecording) {
            try {
              ...
              // drawImage falla en iOS con grabaci├│n nativa ÔåÆ canvas se queda negro
              fillBlack();
            }
          } else {
            fillBlack();
    ...
    return (
      ...
      >
        ...
        {/* Canvas overlay anti-grabaci├│n: dibuja frames del video o negro si grabando */}
        <canvas
          ref={canvasRef}
        ...
        {/* Gran bot├│n de play inicial (solo cuando est├í pausado y al inicio) ÔÇö Netflix rounded rectangle */}
        ...
        {/* ===== CONTROLES MOBILE (< 1024px) ÔÇö NETFLIX STYLE ===== */}
        <div
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
            <div className="flex items-center gap-5 pointer-events-auto">
              ...
              {/* Play/Pause ÔÇö rounded rectangle Netflix style */}
              <button
          ...
          {/* Esquina superior izquierda ÔÇö Fullscreen */}
          ...
          {/* Esquina superior derecha ÔÇö Volumen + PiP */}
          ...
          {/* Barra inferior ÔÇö Netflix style: progress + time */}
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent px-3 pt-6 pb-3 pointer-events-auto z-20">
            {/* Progress bar ÔÇö mobile: big touch target + drag seek */}
            <div className="mb-1.5">
              ...
              >
                ...
                <div className="absolute top-0 left-0 h-[3px] bg-emerald-500 rounded-full" style={{ width: `${duration > 0 ? (currentTime / duration) * 100 : 0}%` }}>
                  {/* Scrubber dot ÔÇö always visible on mobile, hover on desktop */}
        ...
        >
          ...
          {/* CENTRO - Play/Pause + Seek ÔÇö Netflix rounded rectangle style */}
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                ...
                <div className="relative">
                  ...
                  {showAdditionalMenu && (
                    <div className="absolute bottom-full right-0 mb-2 bg-black/90 rounded-lg shadow-xl border border-gray-700 overflow-hidden z-50 w-40">
                      <button className="w-full px-4 py-2 text-left text-sm text-white hover:bg-gray-700" onClick={(e) => e.stopPropagation()}>Configuraci├│n</button>
  ...
  // ÔöÇÔöÇ Placeholder (sin video disponible) ÔöÇÔöÇ
  return (
    <div
      ref={containerRef}
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center gap-4">
        ...
        <p className="text-white/40 text-xs text-center px-8 max-w-md">
          El video de esta clase estar├í disponible pronto.
        </p>
      </div>
