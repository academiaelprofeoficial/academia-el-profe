...
export interface MergedDashboardCourse {
  readonly id: string;
...
export function mergeDashboardCourses(sanityCourses: SanityCourse[] | null): MergedDashboardCourse[] {
  ...
  return utpCourses.map((sc) => {
    const mock = DASHBOARD_COURSES.find((m) => m.id === sc.slug);
    ...
    return {
      ...
      color: mock?.color || 'bg-brand-primary',
...
function PaymentButtons({
  ...
  readonly course: MergedDashboardCourse;
...
function CourseCard({ course, isPurchased, index }: { readonly course: MergedDashboardCourse; readonly isPurchased: boolean; readonly index: number }) {
  const hex = course.cardColor;
  const { user } = useAuth();
...
function DashboardUTPContent({ sanityCourses }: { readonly sanityCourses: SanityCourse[] | null }) {
  ...
  const courses = mergeDashboardCourses(sanityCourses);
...
export function DashboardUTPPageClient({ sanityCourses }: { readonly sanityCourses: any[] | null }) {
  return (
    <Suspense fallback={<div className="p-8 text-center"><Loader2 className="h-6 w-6 animate-spin mx-auto text-brand-primary" /></div>}>
      <DashboardUTPContent sanityCourses={sanityCourses} />
    </Suspense>
  );
}